...
| `ModuleNotFoundError: No module named '...'` or `ImportError: Missing optional dependency '...'` | A library is not installed. Everything we use comes with Anaconda, so this normally means Jupyter was started outside Anaconda. | Start Jupyter from Anaconda Navigator. If a package is really missing, install it: run `!pip install <name>` in a notebook cell, or `conda install <name>` from Navigator's *Environments* tab (the `pandas` session shows this). |
